...
struct RangeSet {
    ...
    // Anonymous executable regions as they existed at JNI_OnLoad. Captured
    // separately (and once, early) because membership here is a statement about
    // WHEN a region appeared, not what it is.
    std::vector<Range> anon_exec_baseline;
};
...
bool path_is_oat(const char* path, size_t path_len) {
    if (path_len < 5) return false;
    if (std::strncmp(path + path_len - 4, ".oat", 4) == 0 ||
        std::strncmp(path + path_len - 4, ".art", 4) == 0)
        return true;
    // ".odex" is FIVE characters. The previous form compared 4 bytes from
    // path_len-4 — i.e. the path's last four chars ("odex") against ".odex"'s
    // first four (".ode") — so a real `base.odex` never matched and AOT-compiled
    // app code classified UNKNOWN, which Vector A treats as proof-positive.
    if (path_len < 6) return false;
    return std::strncmp(path + path_len - 5, ".odex", 5) == 0;
}
...
void parse_proc_self_maps(RangeSet* set) {
    // B3: raw-syscall read — the JIT/oat classification this feeds decides
    // which executable ranges the inline-hook detectors watch; a libc stdio
    // hook silently removing a `jit` line here would exempt an injected cache.
    std::string maps;
    if (!svc::read_file("/proc/self/maps", &maps)) {
        RLOGW("F18 ranges: raw read of /proc/self/maps failed");
        return;
    }
...
const char* classification_name(Classification c) {
    switch (c) {
        ...
        case Classification::IN_ANON_EXEC_BASELINE: return "anon_exec_baseline";
...
Classification classify(const void* addr) {
    ...
    // Checked LAST: a named region always wins, so this only ever rescues an
    // address that no path- or label-based bucket could account for.
    if (addr_in(g_ranges.anon_exec_baseline, a)) return Classification::IN_ANON_EXEC_BASELINE;
...
bool maps_line_is_anon_exec(const char* line, uintptr_t* start, uintptr_t* end) {
    ...
    if (std::sscanf(line, "%lx-%lx %4s %lx %15s %lu %n",
                    &a, &b, perms, &offset, dev, &inode, &consumed) < 6) {
        return false;
    }
    if (!perms_executable(perms)) return false;
    if (b <= a) return false;

    // Anonymous means NOTHING follows the inode but whitespace. A `[label]` (e.g.
    // "[anon:jit-code-cache]") is NOT anonymous for our purposes: it is named, and
    // the label-based JIT bucket already claims it.
    for (const char* p = line + consumed; *p; ++p) {
        if (*p != ' ' && *p != '\t' && *p != '\n' && *p != '\r') return false;
    }
    *start = static_cast<uintptr_t>(a);
    *end = static_cast<uintptr_t>(b);
    return true;
}
...
size_t initialize_anon_exec_baseline() {
    ...
    std::call_once(once, [] {
        ...
        while (cur.next(&line)) {
            uintptr_t a = 0, b = 0;
            if (!maps_line_is_anon_exec(line.c_str(), &a, &b)) continue;
            g_ranges.anon_exec_baseline.push_back({a, b});
        }
    });
    return g_ranges.anon_exec_baseline.size();
}
