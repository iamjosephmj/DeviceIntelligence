...
size_t scan_jni_entry(JniEntryScanEntry* out, size_t out_capacity) {
    ...
    bool late_bound = false;   // a null baseline was adopted this round (see below)
    for (size_t i = 0; i < n; ++i) {
        ...
        if (intact) {
            ...
            e.snapshot_class = e.snapshot_entry != nullptr
                                   ...
                                   : Classification::UNKNOWN;
            if (e.snapshot_entry == nullptr && e.live_entry != nullptr) {
                // LATE JNI BINDING — not a hook. A declared-native method whose
                // `data_` is still NULL at JNI_OnLoad simply has not been linked
                // yet; ART resolves it lazily on first invocation. Reporting
                ...
                // libart nor the JIT code cache), so a live_class corroboration
                // would NOT have caught this — the null baseline is the defect.
                //
                // Adopt the first resolved value as the baseline instead, so the
                // slot becomes monitored from now on: any LATER change is real
                // drift. Self-limiting — once adopted the baseline is non-null, so
                // this branch cannot be taken again for this slot.
                values[i] = e.live_entry;
                late_bound = true;
                e.snapshot_entry = e.live_entry;
                e.snapshot_class = e.live_class;
                e.drifted = false;
                RLOGI("F18 Vector E: late JNI binding %s -> %p (baseline adopted)",
                      e.short_id, e.live_entry);
            } else {
    ...
    if (intact && late_bound) {
        // The values page changed (late-bound baselines adopted), so the integrity
        // hash must be refreshed — otherwise the NEXT scan reads the page as
        // tampered and suppresses drift for every slot.
        store_hash_for_current_values();
    }
