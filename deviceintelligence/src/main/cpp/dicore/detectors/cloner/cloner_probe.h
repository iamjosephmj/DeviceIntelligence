...
/**
 * THIS app's base.apk, selected by package component rather than by being first.
 *
 * [read_apk_path_from_maps] returns whichever base.apk is mapped earliest, which is
 * not necessarily ours: an Xposed module's APK is mapped into the target process and
 * can sort first. Reading our own assets through that path silently reads the WRONG
 * apk — measured on-device, where an enabled module starved the licence blob and the
 * SDK went silent. Anything that must read our own APK uses this.
 */
...
// (i.e. "this path belongs to package X"). Used by the native cloner
// verdict so the apk-path decision is made in C++, not Kotlin.
