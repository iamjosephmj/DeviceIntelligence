#include "dicore/detectors/native_integrity/process/baseline.h"
...
void parse_proc_maps(
    ...
    // B3: raw-syscall read — this baseline defines which executable mappings
    // the whole native-integrity trust inheritance rests on; a libc stdio hook
    // could drop the one line covering the injected library.
    std::string maps;
    ...
    while (cur.next(&line)) {
        ...
        int consumed = 0;
        if (std::sscanf(line.c_str(), "%lx-%lx %7s %lx %15s %lu %n",
