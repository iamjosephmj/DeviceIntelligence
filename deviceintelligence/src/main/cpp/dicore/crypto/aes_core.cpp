#include "dicore/crypto/aes_core.h"

#include <cstring>

namespace dicore::crypto {
namespace {

const uint8_t kSbox[256] = {
  
};

inline uint8_t xtime(uint8_t x) {
    return static_cast<uint8_t>((x << 1) ^ (((x >> 7) & 1) * 0x1b));
}

} // namespace

void aes256_expand_key(const uint8_t key[32], uint8_t rk[kAes256RoundKeyBytes]) {
    // Nk=8, Nr=14 -> 60 words = 240 bytes.
    std::memcpy(rk, key, 32);
    uint8_t rcon = 1;
    for (int i = 8; i < 60; ++i) {
        uint8_t t[4];
        int prev = (i - 1) * 4;
        t[0] = rk[prev + 0]; t[1] = rk[prev + 1]; t[2] = rk[prev + 2]; t[3] = rk[prev + 3];
        if (i % 8 == 0) {
            uint8_t tmp = t[0];                         // RotWord
            t[0] = kSbox[t[1]] ^ rcon;                  // SubWord + Rcon
            t[1] = kSbox[t[2]];
            t[2] = kSbox[t[3]];
            t[3] = kSbox[tmp];
            rcon = xtime(rcon);
        } else if (i % 8 == 4) {
            t[0] = kSbox[t[0]]; t[1] = kSbox[t[1]];     // SubWord
            t[2] = kSbox[t[2]]; t[3] = kSbox[t[3]];
        }
        int cur = i * 4, back = (i - 8) * 4;
        rk[cur + 0] = rk[back + 0] ^ t[0];
        rk[cur + 1] = rk[back + 1] ^ t[1];
        rk[cur + 2] = rk[back + 2] ^ t[2];
        rk[cur + 3] = rk[back + 3] ^ t[3];
    }
}

void aes256_encrypt_block(const uint8_t rk[kAes256RoundKeyBytes],
                          const uint8_t in[16], uint8_t out[16]) {
    uint8_t s[16];
    for (int i = 0; i < 16; ++i) s[i] = in[i] ^ rk[i];   // AddRoundKey (round 0)

    for (int round = 1; round <= 14; ++round) {
        // SubBytes
        for (int i = 0; i < 16; ++i) s[i] = kSbox[s[i]];
        // ShiftRows (column-major state: s[r + 4c])
        uint8_t t[16];
        for (int r = 0; r < 4; ++r)
            for (int c = 0; c < 4; ++c)
                t[r + 4 * c] = s[r + 4 * ((c + r) % 4)];
        std::memcpy(s, t, 16);
        // MixColumns (skip on final round 14)
        if (round != 14) {
            for (int c = 0; c < 4; ++c) {
                uint8_t* col = s + 4 * c;
                uint8_t a0 = col[0], a1 = col[1], a2 = col[2], a3 = col[3];
                col[0] = xtime(a0) ^ (xtime(a1) ^ a1) ^ a2 ^ a3;
                col[1] = a0 ^ xtime(a1) ^ (xtime(a2) ^ a2) ^ a3;
                col[2] = a0 ^ a1 ^ xtime(a2) ^ (xtime(a3) ^ a3);
                col[3] = (xtime(a0) ^ a0) ^ a1 ^ a2 ^ xtime(a3);
            }
        }
        // AddRoundKey
        const uint8_t* rkr = rk + 16 * round;
        for (int i = 0; i < 16; ++i) s[i] ^= rkr[i];
    }
    std::memcpy(out, s, 16);
}

} // namespace dicore::crypto
