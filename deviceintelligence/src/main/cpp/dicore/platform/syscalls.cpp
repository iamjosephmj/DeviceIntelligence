...
#if defined(__arm__)
// process_vm_readv via libc on arm32 (see the policy block comment above).
// Counts declared as unsigned long to stay identical to bionic's
// <sys/uio.h> declaration (glibc's size_t counts are the same type on
// every ABI where both are visible).
extern "C" ssize_t process_vm_readv(pid_t, const struct iovec*, unsigned long,
...
constexpr long NR_uname   = 160;
constexpr long NR_getpid  = 172;
constexpr long NR_pvmread = 270;  // process_vm_readv
constexpr long NR_write   = 64;
...
constexpr long NR_uname   = 63;
constexpr long NR_getpid  = 39;
constexpr long NR_pvmread = 310;  // process_vm_readv
constexpr long NR_write   = 1;
...
// reads on 32-bit ARM that they cannot on 64-bit — a known
// armeabi-v7a coverage limitation.
...
// Single-shot raw read: -1 on error (errno_out set), 0 on EOF, >0 partial.
// The retry loop lives in raw_read_full / svc_io's state machine.
ssize_t raw_read(int fd, void* buf, size_t count, int* errno_out) {
    if (count == 0) return 0;
    if (!buf) { if (errno_out) *errno_out = EINVAL; return -1; }
#if defined(__arm__)
    ssize_t r = ::read(fd, buf, count);
    if (r < 0 && errno_out) *errno_out = errno;
    return r;
#else
    long r = do_syscall6(NR_read, (long)fd, (long)buf, (long)count, 0, 0, 0);
    if (is_kernel_error(r)) {
        if (errno_out) *errno_out = (int)(-r);
        return -1;
    }
...
ssize_t raw_write(int fd, const void* buf, size_t count, int* errno_out) {
    ...
    return r;
#else
    long r = do_syscall6(NR_write, (long)fd, (long)buf, (long)count, 0, 0, 0);
    if (is_kernel_error(r)) {
        if (errno_out) *errno_out = (int)(-r);
        return -1;
    }
...
ssize_t raw_read_full(int fd, void* buf, size_t count, int* errno_out) {
    ...
    size_t total = 0;
    while (total < count) {
        ssize_t r = raw_read(fd, p + total, count - total, errno_out);
        if (r < 0) return -1;
        if (r == 0) break; // EOF
        total += (size_t)r;
    }
...
ssize_t raw_self_read(const void* addr, size_t len, void* dst, int* errno_out) {
    if (len == 0) return 0;
    if (!addr || !dst) { if (errno_out) *errno_out = EINVAL; return -1; }
#if defined(__arm__)
    // Same arm policy as the rest of this file (see the block comment above).
    struct iovec local{ dst, len }, remote{ const_cast<void*>(addr), len };
    ssize_t r = process_vm_readv(::getpid(), &local, 1, &remote, 1, 0);
    if (r < 0 && errno_out) *errno_out = errno;
    return (r == (ssize_t)len) ? r : -1;
#else
    long pid = do_syscall6(NR_getpid, 0, 0, 0, 0, 0, 0);
    struct iovec local{ dst, len }, remote{ const_cast<void*>(addr), len };
    long r = do_syscall6(NR_pvmread,
                         pid,
                         (long)&local, 1,
                         (long)&remote, 1,
                         0);
    ...
    return (r == (long)len) ? (ssize_t)r : -1;  // reject partial copies
#endif
}
...
int raw_getpid(int* errno_out) {
    ...
    return r;
#else
    long r = do_syscall6(NR_getpid, 0, 0, 0, 0, 0, 0);
    if (is_kernel_error(r)) {
        if (errno_out) *errno_out = (int)(-r);
        return -1;
    }
...
int raw_uname(struct utsname* out, int* errno_out) {
    if (out == nullptr) { if (errno_out) *errno_out = EINVAL; return -1; }
#if defined(__arm__)
    // 32-bit arm goes through libc, matching how raw_fstat_size handles it.
    if (::uname(out) < 0) { if (errno_out) *errno_out = errno; return -1; }
#else
    long r = do_syscall6(NR_uname, (long)out, 0, 0, 0, 0, 0);
    if (is_kernel_error(r)) {
        if (errno_out) *errno_out = (int)(-r);
        return -1;
    }
